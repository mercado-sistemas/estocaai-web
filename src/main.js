...
/* A janela Incluir/Alterar Produto foi migrada para React (src/produto/), pela
   segurança: ela exibe descrição de produto e dados de NF-e de terceiro, e no
   JSX o texto é escapado automaticamente. O main.js continua dono do estado —
   passa ao componente um contexto com a API e os dados de que ele precisa. */
...
/* Tela de Clientes migrada para React (src/cliente/), pela segurança: nome,
   fantasia, CPF/CNPJ, cidade e observação são digitados pelo usuário e antes
   iam para innerHTML. O main.js só monta o componente e passa a API. */
function janelaClientes() {
  fecharJanela();
  abrirModalCliente({ apiFetch, toast });
}
